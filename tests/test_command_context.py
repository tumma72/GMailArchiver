...
class TestWithContextDecorator:
    ...
    def test_basic_decorator(self) -> None:
        ...
        with patch("gmailarchiver.command_context.OutputManager") as MockOutput:
    ...
    def test_requires_gmail_success(self, tmp_path: Path) -> None:
        ...
        with (
            patch("gmailarchiver.command_context.OutputManager") as MockOutput,
            patch("gmailarchiver.command_context.GmailAuthenticator") as MockAuth,
            patch("gmailarchiver.command_context.GmailClient") as MockGmail,
        ):
            mock_output = MagicMock(spec=OutputManager)
            MockOutput.return_value = mock_output
            mock_auth = MagicMock()
