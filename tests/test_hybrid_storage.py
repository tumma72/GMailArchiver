...
from gmailarchiver.hybrid_storage import HybridStorage, HybridStorageError, IntegrityError
...
class TestHybridStorageInitialization:
    ...
    def test_init_with_valid_db_manager(self, db_manager: DBManager) -> None:
    ...
    def test_init_creates_staging_area(self, db_manager: DBManager, temp_dir: Path) -> None:
    ...
    def test_cleanup_staging_area_on_del(self, db_manager: DBManager) -> None:
...
class TestArchiveMessage:
    ...
    def test_archive_message_calculates_offset_correctly(
        self, db_manager: DBManager, sample_email_message: email.message.Message, mbox_path: Path
    ) -> None:
        ...
        msg1_data = db_manager.get_message_by_gmail_id("msg1")
    ...
    def test_archive_message_calculates_length_correctly(
        self, db_manager: DBManager, sample_email_message: email.message.Message, mbox_path: Path
    ) -> None:
    ...
    def test_archive_message_with_gzip_compression(
    ...
    def test_archive_message_with_lzma_compression(
        self, db_manager: DBManager, sample_email_message: email.message.Message, temp_dir: Path
    ) -> None:
    ...
    def test_archive_message_with_zstd_compression(
        self, db_manager: DBManager, sample_email_message: email.message.Message, temp_dir: Path
    ) -> None:
    ...
    def test_archive_message_lock_file_management(
        self, db_manager: DBManager, sample_email_message: email.message.Message, mbox_path: Path
    ) -> None:
    ...
    def test_archive_message_cleans_staging_file(
        self, db_manager: DBManager, sample_email_message: email.message.Message, mbox_path: Path
    ) -> None:
...
class TestConsolidateArchives:
    ...
    def test_consolidate_without_deduplication(self, db_manager: DBManager, temp_dir: Path) -> None:
    ...
    def test_consolidate_with_deduplication(self, db_manager: DBManager, temp_dir: Path) -> None:
    ...
    def test_consolidate_recalculates_offsets(self, db_manager: DBManager, temp_dir: Path) -> None:
        ...
        orig_msg2 = db_manager.get_message_by_gmail_id("msg2")
        ...
        # Get new offsets
        new_msg1 = db_manager.get_message_by_gmail_id("msg1")
        new_msg2 = db_manager.get_message_by_gmail_id("msg2")
    ...
    def test_consolidate_validates_result(self, db_manager: DBManager, temp_dir: Path) -> None:
    ...
    def test_consolidate_with_compressed_archives(
    ...
    def test_consolidate_rollback_on_failure(self, db_manager: DBManager, temp_dir: Path) -> None:
        ...
        current_data = db_manager.get_message_by_gmail_id("msg1")
...
class TestValidation:
    ...
    def test_validate_message_consistency_valid_message(
        self, db_manager: DBManager, sample_email_message: email.message.Message, mbox_path: Path
    ) -> None:
    ...
    def test_validate_message_consistency_missing_in_database(
        ...
        with pytest.raises(IntegrityError, match="not in database"):
            storage._validate_message_consistency("nonexistent")

    def test_validate_message_consistency_missing_in_mbox(
        self, db_manager: DBManager, sample_email_message: email.message.Message, mbox_path: Path
    ) -> None:
    ...
    def test_validate_archive_consistency_count_mismatch(
        self, db_manager: DBManager, mbox_path: Path
    ) -> None:
    ...
    def test_validate_archive_consistency_orphaned_message(
        self, db_manager: DBManager, mbox_path: Path
    ) -> None:
...
class TestAtomicity:
    ...
    def test_transaction_commits_on_full_success(
        self, db_manager: DBManager, sample_email_message: email.message.Message, mbox_path: Path
    ) -> None:
        ...
        msg_data = new_db.get_message_by_gmail_id("msg123")
        assert msg_data is not None
        assert msg_data["gmail_id"] == "msg123"
        new_db.close()
    ...
    def test_partial_write_detection(self, db_manager: DBManager, mbox_path: Path) -> None:
...
class TestErrorHandling:
    ...
    def test_corrupt_source_archive_in_consolidation(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
...
class TestEdgeCasesAndCoverage:
    ...
    def test_decompress_corrupt_gzip(self, db_manager: DBManager, temp_dir: Path) -> None:
    ...
    def test_decompress_with_invalid_compression_format(
        ...
        with pytest.raises(ValueError, match="Unsupported compression format"):
            storage._decompress_file(source, dest, "bzip2")

    def test_compress_with_invalid_compression_format(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
        ...
        # Should raise ValueError for unsupported compression
        with pytest.raises(ValueError, match="Unsupported compression format"):
            storage._compress_file(source, dest, "bzip2")

    # ============ Validation Edge Cases ============

    def test_validate_archive_consistency_decompression_fails(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
        ...
        # Validation should fail
        with pytest.raises(IntegrityError, match="Failed to decompress"):
            storage._validate_archive_consistency(corrupt_gz)

    def test_collect_messages_decompression_fails(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
        ...
        with pytest.raises(HybridStorageError, match="Failed to decompress"):
            storage._collect_messages([corrupt_gz])

    # ============ Consolidation Edge Cases ============

    def test_consolidate_validation_missing_messages(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
    ...
    def test_consolidate_validation_unexpected_messages(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
    ...
    def test_context_manager_enter(self, db_manager: DBManager) -> None:
    ...
    def test_context_manager_exit_cleanup(self, db_manager: DBManager) -> None:
    ...
    def test_cleanup_staging_area_with_permission_error(self, db_manager: DBManager) -> None:
    ...
    def test_cleanup_staging_area_with_missing_attribute(self, db_manager: DBManager) -> None:
    ...
    def test_extract_body_preview_non_multipart_with_exception(self, db_manager: DBManager) -> None:
    ...
    def test_extract_body_preview_multipart_with_exception(self, db_manager: DBManager) -> None:
    ...
    def test_extract_body_preview_very_short(self, db_manager: DBManager) -> None:
    ...
    def test_archive_message_with_unusual_compressed_extension(
        self, db_manager: DBManager, sample_email_message: email.message.Message, temp_dir: Path
    ) -> None:
    ...
    def test_archive_message_with_orphaned_lock_file(
        self, db_manager: DBManager, sample_email_message: email.message.Message, mbox_path: Path
    ) -> None:
    ...
    def test_archive_message_to_new_file_offset_zero(
        self, db_manager: DBManager, sample_email_message: email.message.Message, mbox_path: Path
    ) -> None:
    ...
    def test_validate_message_missing_archive_file(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
    ...
    def test_deduplicate_with_duplicate_rfc_message_ids(self, db_manager: DBManager) -> None:
    ...
    def test_validate_consolidation_with_temp_file_cleanup(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
    ...
    def test_cleanup_staging_area_iterdir_exception(self, db_manager: DBManager) -> None:
    ...
    def test_extract_body_preview_no_text_plain_part(self, db_manager: DBManager) -> None:
    ...
    def test_extract_body_preview_payload_not_bytes(self, db_manager: DBManager) -> None:
    ...
    def test_consolidate_with_compression_output(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
    ...
    def test_bulk_write_database_error_rollback(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
    ...
    def test_archive_message_with_lock_file_cleanup(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
    ...
    def test_archive_message_with_compression_and_various_formats(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
    ...
    def test_consolidate_with_archive_file_fallback(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
    ...
    def test_archive_message_compression_extension_detection(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
    ...
    def test_consolidate_with_lock_file_cleanup(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
    ...
    def test_bulk_update_archive_locations_error(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
    ...
    def test_consolidate_rollback_on_db_failure(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
    ...
    def test_consolidate_staging_cleanup_failure(
        self, db_manager: DBManager, temp_dir: Path
    ) -> None:
...
def test_hybrid_storage_malformed_email_body(temp_dir, v11_db):
...
def test_hybrid_storage_rollback_cleans_staging(temp_dir, v11_db):
    ...
    with patch.object(
        ...
        with DBManager(str(v11_db)) as db:
            messages = db.get_all_messages_for_archive(str(output))
...
def test_bulk_write_cleanup_on_error_path(db_manager: DBManager, temp_dir: Path) -> None:
...
def test_bulk_write_finally_block_exceptions(db_manager: DBManager, temp_dir: Path) -> None:
...
def test_consolidate_rollback_on_general_exception(db_manager: DBManager, temp_dir: Path) -> None:
...
def test_consolidate_finally_block_unlock_close(db_manager: DBManager, temp_dir: Path) -> None:
...
def test_consolidate_rollback_failure_logging(db_manager: DBManager, temp_dir: Path) -> None:
...
def test_consolidate_staging_cleanup_exception(db_manager: DBManager, temp_dir: Path) -> None:
